 ...
 *
 * §7.3's delivery re-check asks the same question of a grant's documents, but
 * answers it from the join `conferredSelect` already runs rather than from
 * here — it has the rows in hand and a second read would be the round trip
 * that check exists to avoid. The *rule* is still named once, in `proposeFrom`,
